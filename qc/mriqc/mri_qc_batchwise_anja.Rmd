...
## folders
```{r}
input_BIDS <- "/mnt/spaceC/Bids_copy/bids/sourcedata"
output_MRIQC <- "/media/niklas/Storage3/mriqc"
# here are the files copied and removed.
temp_batch_MRIQC <- "/media/niklas/Storage3/mriqc_temp/"

```


```{r}
input_df <- tibble(path = list.files(input_BIDS, full.names = TRUE, recursive = TRUE) %>% str_subset("sub-"),
...
```{r}
output_df <- tibble(path = list.files(output_MRIQC, full.names = TRUE, recursive = TRUE) %>% str_subset("sub-"),
       sub = str_extract(path, "sub-[:digit:]{5}"),
       session = str_extract(path, "ses-s[:digit:]{1}"),
       sub_session = str_extract(path, "sub-[:digit:]{5}_ses-s[:digit:]{1}"))
...
for(i in 1:length(subjects)){
  ...
  docker_string = paste0("docker run --rm --read-only --tmpfs /run --tmpfs /tmp -v ", 
